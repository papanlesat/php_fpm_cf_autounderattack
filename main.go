...
func main() {
	...
	if err != nil {
		...
		// emailConfig remains nil if not properly configured.
	}
	...
	// Iterate over CPU usage data for each user.
	for user, usage := range cpuUsage {
		...
		// Create a Cloudflare service instance for the given zone.
		...
		// If CPU usage exceeds threshold, activate under_attack (if not already active).
		if usage > cpuThreshold {
			...
			// If CPU usage is below threshold and mode is still under_attack, disable it.
			if currentLevel == "under_attack" {
